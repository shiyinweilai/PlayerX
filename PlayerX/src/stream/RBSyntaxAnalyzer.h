 ...
 *     { name, value } 名值对列表；不解析 slice 层（数据量大且无必要）。
 ...
 *
 * 数据来源优先级：
 *   1) 容器 extradata（mp4 等）：avformat 已拿到，直接喂 CBS read_extradata；
 *   2) 裸 AnnexB 文件（.265/.266/.h264）：读前 N MB 扫 SPS/PPS/VPS NAL，
 *      逐 NAL 喂 CBS read（带 nal header），trace 收集名值对。
 *
