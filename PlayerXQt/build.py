#!/usr/bin/env python3
"""
PlayerXQt build.py — Qt + QML 版本构建脚本（macOS 原生 / macOS→Windows 交叉）
...
设计要点:
    * 产物严格分离：
        macOS   → PlayerXQt/build/out/      install/
        Windows → PlayerXQt/build/out_win/  install_win/
...
def find_ffmpeg(target: str) -> str:
    ...
    # 优先 PlayerX/build/ffmpeg/install[_win]
    candidates = [
...
def post_build(target: str, qt_dir: str):
    ...
    if target == "windows":
        idir = install_dir_for(target)
        exe_path = os.path.join(idir, "bin", "PlayerXQt.exe")
...
# ─── 主入口 ────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="PlayerXQt 构建脚本（macOS / macOS→Windows 交叉）")
