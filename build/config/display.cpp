...
Display::Display(const int display_number,
  ...
  const int create_window_flags = SDL_WINDOW_SHOWN | SDL_WINDOW_ALWAYS_ON_TOP;
 ...
 // 简化图标设置：使用极小的单色图标避免编译警告
#ifdef _WIN32
  // 创建64x64的单色图标（黑色背景，白色矩形）
  SDL_Surface* icon_surface = SDL_CreateRGBSurface(0, 64, 64, 32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000);
  SDL_FillRect(icon_surface, nullptr, SDL_MapRGB(icon_surface->format, 0, 0, 0)); // 黑色背景
  SDL_Rect icon_rect = {16, 16, 32, 32};
  SDL_FillRect(icon_surface, &icon_rect, SDL_MapRGB(icon_surface->format, 255, 255, 255)); // 白色矩形
  ...
  SDL_FillRect(icon_surface, nullptr, SDL_MapRGB(icon_surface->format, 128, 128, 128));
  SDL_SetWindowIcon(window_, icon_surface);
  SDL_FreeSurface(icon_surface);
#endif
  ...
  if (show_hud_) {
    ...
    if (show_left_) {
      ...
      if (mode_ == Mode::VSTACK) {
        ...
        // 移除文件路径显示: render_text(line1_y_, line2_y_, left_text_texture_, left_text_width_, left_text_height_, border_extension_, true);
      } else {
        // 移除文件路径显示: render_text(line1_y_, line1_y_, left_text_texture_, left_text_width_, left_text_height_, border_extension_, true);
    ...
    if (show_right_) {
      ...
      // 移除文件路径显示: render_text(text1_x, text1_y, right_text_texture_, right_text_width_, right_text_height_, border_extension_, false);
